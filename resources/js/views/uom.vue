<template>
    <div>
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h4 class="mb-0 text-gray-800">UOM</h4>
            <router-link to="/uom/new" class="d-sm-inline-block btn btn-primary shadow-sm text-right">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="form-group col-md-4 col-lg-3">
                        ...
                        <input type="text" class="form-control" id="uom_name" name="uom_name" v-model="search.uom_name">
                    </div>
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
              <div class="table-responsive" v-if="uom_count > 0">
                    <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">  <!--kamlesh-->
                        ...
                        <tbody>
                            <tr v-for="uom,index in uoms.data">
                                <td class="text-center">{{((currentPage * perPage) - perPage) + (index+1)}}</td>
                                <td class="mm-txt text-center">{{uom.uom_name}}</td>

                                <td v-if="uom.is_active == 1" class="text-center">
                                ...
                                <td class="text-center">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                            ...
                                            <router-link tag="span" :to="'/uom/edit/' + uom.id" >
                                                <a href="#" title="Edit/View" class="">
                                                    <i class="fas fa-edit"></i>
                                                </a>&nbsp;
                                            </router-link>
                                            ...
                                            <a class="dropdown-item">
                                                <a class="badge badge-primary text-white"  @click="changeStatus(uom.id,'inactive')" v-if="uom.is_active == 1">Inactive</a>
                                                <a class="badge badge-primary text-white" @click="changeStatus(uom.id,'active')" v-else>Active</a>  
                                            </a>
                                        </div>
                                    </div>
                ...
                <div v-else>
                    <h5 class="text-center m-5">No uom found!</h5>
                </div>
            </div>
                  ...
                  <template v-if="uom_count > 0">
                    <div class="overflow-auto text-center" style="display:inline-block">
                      ...
                      <b-pagination
                        ...
                        last-text="Last"
                      >
                        <template v-slot:first-text><span class="text-success" v-on:click="getUoms(1)">First</span></template>
                        <template v-slot:prev-text><span class="text-danger" v-on:click="getUoms(currentPage)">Prev</span></template>
                        <template v-slot:next-text><span class="text-warning" v-on:click="getUoms(currentPage)">Next</span></template>
                        <template v-slot:last-text><span class="text-info" v-on:click="getUoms(pagination.last_page)">Last</span></template>
                        <template v-slot:ellipsis-text>
                        </template>
                        <template v-slot:page="{ page, active }">
                          <span v-if="active"><b>{{ page }}</b></span>
                          <span v-else v-on:click="getUoms(page)">{{ page }}</span>
                        </template>
                      </b-pagination>
                    </div>
                  </template>
    ...
    export default {
        ...
        methods: {
            ...
            changeStatus(id,status) {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $("#loading").show();
                        axios
                        .get("/uom_status/" + uom_id + "/" + active)
                        .then(function(response) {
                            $("#loading").hide();
                            app.getUoms(app.currentPage);
                            swal("Success! UOM has been updated as " + active+".", {
                            icon: "success"
                          });
                        });
            ...
            getUoms(page = 1) {
                ...
                axios.get("/uom?page=" + page + search).then(function(response) {
                    $("#loading").hide();
                    let data = response.data.data;
                    app.uoms = data;
                    app.uom_count = app.uoms.data.length;
